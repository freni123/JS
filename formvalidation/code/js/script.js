...
function validateForm() {
    ...
    try {
        ...
        switch (err) {
            case 'firstname cannot be a number':
                Fnameerror.textContent = err;
                break;
            case 'firstname must be at least 3 characters':
                Fnameerror.textContent = err;
                break;
            case 'firstname must be less than 10 characters':
                Fnameerror.textContent = err;
                break;
            case 'lastname cannot be a number':
                lanmeerror.textContent = err;
                break;
            case 'lastname must be at least 3 characters':
                lanmeerror.textContent = err;
                break;
            case 'lastname must be 10 characters':
                lanmeerror.textContent = err;
                break;
            case 'invalid email format':
                Emailerror.textContent = err;
                break;
            case 'phoneno must be 10 number':
                Phonenoerror.textContent = err;
                break;
            case 'invalid birth date format':
    ...
    data.map(function (item) {
        ...
        addressCell.innerHTML = item.address;
            ...
            if(item.gender ==='male'){
                ...
                malenameCell.innerHTML=item.firstName.concat(" ", item.lastName);
            }
